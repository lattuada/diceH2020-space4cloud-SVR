...
## Run a test with training on some given R query and test with another R query.
...
TRAIN_QUERIES_COUPLES = {{'R5'}, {'R2'}, {'R3'}, {'R4'}, {'R1', 'R2', 'R4'}, {'R2', 'R5'}, {'R3', 'R4'}};
TEST_QUERIES_COUPLES = {{'R2'}, {'R5'}, {'R4'}, {'R3'}, {'R3'}, {'R3'}, {'R2'}};
DESCRIPTIONS = {"(Fast - Fast)", "(Fast - Fast)", "(Slow - Slow)", "(Slow - Slow)", "(Fast and slow - Average)", "(Fast - Slow)", "(Slow - Fast)"};
...
SECTION_CAPTION = 'Query comparison only ncores';
LATEX_FILENAME = 'query_comparison_only_ncores';
OUTPUT_FOLDER_SUFFIX = 'ONLY_NCORES';
...
if OUTPUT_LATEX
	flatex = fopen(cstrcat('latex_output/', LATEX_FILENAME, '.tex'), 'w');
	fprintf(flatex, cstrcat('\\newpage\n', ...
							'\\section{', SECTION_CAPTION, '}\n'));
end


for query__couple_id = 1:length(TRAIN_QUERIES_COUPLES)

	TRAIN_QUERIES = TRAIN_QUERIES_COUPLES{query__couple_id};
	TEST_QUERIES = TEST_QUERIES_COUPLES{query__couple_id};

	fprintf("Starting query: %s\n", cstrcat(strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ',')));
	fflush(stdout);
	fprintf(flatex, cstrcat('\\subsection{', strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','), DESCRIPTIONS{query__couple_id} ,'}\n'));

	fprintf(flatex, cstrcat('\\subsubsection{', strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','), '}\n'));
	...
	TABLE_CAPTION = cstrcat('Results for ', strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','));
	PLOT_CAPTION = cstrcat('Completion time vs ncores for: ',strjoin(TRAIN_QUERIES, ','), ' $\\rightarrow$ ', strjoin(TEST_QUERIES, ','));
	TABLE_LABEL = cstrcat('tab:', 'coreonly_linear_', strjoin(TRAIN_QUERIES, ','), '_', strjoin(TEST_QUERIES, ','));
	PLOT_LABEL = cstrcat('fig:', 'coreonly_linear_', strjoin(TRAIN_QUERIES, ','), '_', strjoin(TEST_QUERIES, ','));

	% OUTPUT_FOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
	OUTPUT_FOLDER = strcat('outputQC/', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), '_', OUTPUT_FOLDER_SUFFIX, '/');
	...
	if ismember(1, MODELS_CHOSEN)
		...
		options = ['-s 3 -t 0 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
	...
	if ismember(2, MODELS_CHOSEN)
		...
		options = ['-s 3 -d 2 -t 1 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
		model = svmtrain (y_tr, X_tr, options);
	...
	if ismember(3, MODELS_CHOSEN)
		...
		options = ['-s 3 -d 3 -t 1 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
		model = svmtrain (y_tr, X_tr, options);
	...
	if ismember(4, MODELS_CHOSEN)
		...
		options = ['-s 3 -d 4 -t 1 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
		model = svmtrain (y_tr, X_tr, options);
	...
	if ismember(5, MODELS_CHOSEN)
		...
		options = ['-s 3 -d 6 -t 1 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
		model = svmtrain (y_tr, X_tr, options);
	...
	if ismember(6, MODELS_CHOSEN)
		...
		options = ['-s 3 -t 2 -q -h 0 -p ', num2str(eps), ' -c ', num2str(C)];
		model = svmtrain (y_tr, X_tr, options);
	...
	if OUTPUT_LATEX
		...
		fprintf(flatex_plot, cstrcat('\n\\begin {figure}[hbtp]\n', ...
									'\\centering\n', ...
									'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
		...
		if BEST_MODELS
			fprintf(flatex_plot_bestmodels, cstrcat('\n\\begin {figure}[hbtp]\n', ...
													'\\centering\n', ...
													'\\includegraphics[width=\\textwidth]{', OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_'), '_VS_', strjoin(TEST_QUERIES, '_'), '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}, '}\n', ...
													'\\caption{', PLOT_CAPTION, '}\n', ...
													'\\label{', PLOT_LABEL, '}\n', ...
													'\\end {figure}\n'));	
	...
	if ALL_THE_PLOTS
		for col = 1:M
			...
			if LINEAR_REGRESSION
				...
				x = x_denorm;
				y = ylin;
				save(cstrcat(OUTPUT_FOLDER, 'Linear Regression.mat'), 'x', 'y', 'TRAIN_QUERIES', 'TRAIN_QUERIES');
			...
			if SAVE_DATA & ismember(13, FEATURES) & (col == M)
				file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_') , '_VS_', strjoin(TEST_QUERIES, '_') , OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
				print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
			end
	...
	if BEST_MODELS
		...
		if ALL_THE_PLOTS
			...
			for col = 1:M
				...
				if SAVE_DATA & ismember(13, FEATURES) & (col == M)
					file_location = strrep(strcat(OUTPUT_FOLDER, 'plot_', strjoin(TRAIN_QUERIES, '_') , '_VS_', strjoin(TEST_QUERIES, '_') , '_bestmodels', OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{2}), ' ', '');
					print(OUTPUT_FORMATS{PLOT_SAVE_FORMAT}{1}, file_location);
				end
