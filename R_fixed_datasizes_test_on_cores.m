...
TEST_ID = 'fixed_datasizes_test_cores';
...
SECTION_HEADER = 'Fixed datasizes, test on cores, $ncores^{-1}$';   % Header for the latex document
  ...
  for datasize_id = 1:length(DATASIZES)
    ...
    while (length(TRAINING_TO_DO) > 0) & (length(TESTING_TO_DO) > 0)
      try
        ...
        TABLE_CAPTION_TRAIN = cstrcat('Training results for ', QUERY, ' (', DATASIZE, ' GB), testing on ', strjoin(strsplit(int2str(TESTING_CORES)), ', '), ' cores');
        TABLE_CAPTION_TEST = cstrcat('Testing results for ', QUERY, ' (', DATASIZE, ' GB), testing on ', strjoin(strsplit(int2str(TESTING_CORES)), ', '), ' cores');
        ...
        TABLE_LABEL_TRAIN = cstrcat('tab1:', TEST_ID, '_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));
        TABLE_LABEL_TEST = cstrcat('tab2:', TEST_ID, '_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));
        PLOT_LABEL = cstrcat('fig:', TEST_ID, '_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'));

        % OUTPUT_SUBFOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
        OUTPUT_SUBFOLDER = strcat(OUTPUT_FOLDER, upper(TEST_ID), '_', QUERY, '_', DATASIZE, '_', strjoin(strsplit(int2str(TESTING_CORES)), '_'), '/');
