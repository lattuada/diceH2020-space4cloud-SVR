...
for query_id = 1:length(QUERIES)
  ...
  TABLE_CAPTION_TRAIN = cstrcat('Training results for ', QUERY);
  TABLE_CAPTION_TEST = cstrcat('Testing results for ', QUERY);
  PLOT_CAPTION = cstrcat('Completion time vs ncores for ', QUERY);
  TABLE_LABEL_TRAIN = cstrcat('tab1:', TEST_ID, '_', QUERY);
  TABLE_LABEL_TEST = cstrcat('tab2:', TEST_ID, '_', QUERY);
  PLOT_LABEL = cstrcat('fig:', TEST_ID, '_', QUERY);

  % OUTPUT_SUBFOLDER = strcat('output/', QUERY, '_ALL_FEATURES/');
  OUTPUT_SUBFOLDER = strcat(OUTPUT_FOLDER, upper(TEST_ID), '_', QUERY, '/');
  ...
  if ALL_THE_PLOTS
    for col = 1:M
      ...
      % if (DIFF_MEANS & ismember(13, FEATURES) & (col == 11))
